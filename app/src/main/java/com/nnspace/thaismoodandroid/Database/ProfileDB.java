...
public class ProfileDB extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        String CREATE_TABLE = String.format("CREATE TABLE %s (" +
                ...
                "%s varchar(500)," +
                "%s date," +
                "%s date);", ProfileModel.TABLE_NAME,
                ProfileModel.column.USERNAME, ProfileModel.column.DOB, ProfileModel.column.IS_CAFFEINE,
                ProfileModel.column.IS_DRUG,
                ProfileModel.PatientColumn.SEX, ProfileModel.PatientColumn.IS_PREGNANT,
                ProfileModel.PatientColumn.WEIGHT, ProfileModel.PatientColumn.HEIGHT,
                ProfileModel.PatientColumn.BMI, ProfileModel.PatientColumn.disease,
                ProfileModel.column.CREATED_DATE, ProfileModel.column.MODIFIED_DATE);
        db.execSQL(CREATE_TABLE);
    }

    @Override
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        String DROP_FRIEND_TABLE = "DROP TABLE IF EXISTS " + ProfileModel.TABLE_NAME;
