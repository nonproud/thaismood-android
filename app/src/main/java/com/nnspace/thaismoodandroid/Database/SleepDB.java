...
public class SleepDB extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        String CREATE_TABLE = String.format("CREATE TABLE %s (" +
                        ...
                        "%s date NOT NULL " +
                        "%s date NOT NULL " +
                        ");", SleepModel.TABLE_NAME,
                SleepModel.column.ID,
                SleepModel.column.TOTAL,
                SleepModel.column.START,
                SleepModel.column.END,
                SleepModel.column.DATE);
        db.execSQL(CREATE_TABLE);
    }

    @Override
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        String DROP_FRIEND_TABLE = "DROP TABLE IF EXISTS " + SleepModel.TABLE_NAME;
